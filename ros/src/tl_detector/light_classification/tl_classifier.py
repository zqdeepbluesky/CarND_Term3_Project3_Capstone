...
class TLClassifier(object):
    ...
    def __read_tensor_from_image(self, image):
        ...
        sess = tf.Session()
        feed_dict = {image_tf:image}
        tensor = sess.run(normalized, feed_dict=feed_dict)
    ...
    def __predict(self, image):
        ...
        feed_dict = {self.input_operation.outputs[0]: tensor}
        predictions = self.sess.run(self.output_operation.outputs[0],feed_dict=feed_dict)
